...
contract('Airdrop', function(accounts) {
  ...
  it('should have token defined', function() {
    return airdropInstance.tokenInstance.call().then(function(instance) {
      ...
      assert.equal(
        ...
        'Token address should be same as SRPMT'
      );
    });
  });
  ...
  it('should have balance 5 eth', function() {
    assert.equal(
      ...
      web3.eth.getBalance(airdropInstance.address).toNumber(),
      'should have balance 5 eth'
    );
  });

  it('airdrop should fail as no balance is assigned', function() {
    var address = [accounts[1], accounts[2]];
    return airdropInstance
      .doAirDrop(address, 1)
      .then(function(instance) {
  ...
  it('do 1 token airdrop to 2 different address', function() {
    ...
    var account_one_eth_starting_balance = web3.eth
      .getBalance(account_one)
      .toNumber();
    var account_two_eth_starting_balance = web3.eth
      .getBalance(account_two)
      .toNumber();
    var account_three_eth_starting_balance = web3.eth
      .getBalance(account_two)
      .toNumber();
    ...
    return tokenInstance
      ...
      .then(function() {
        ...
        return airdropInstance.doAirDrop(
          address_batch_2,
          amount,
          eth_contract_balance
        );
      ...
      .then(function(balance) {
        ...
        assert.equal(
          ...
          "Amount wasn't sent to the receiver one"
        );
        assert.equal(
          account_two_ending_balance,
          account_two_starting_balance + amount,
          "Amount wasn't correctly sent to the receiver two"
        );
        assert.equal(
          account_three_ending_balance,
          account_three_starting_balance + amount,
          "Amount wasn't correctly sent to the receiver three"
        );
        // fetch final eth balance
        account_one_eth_ending_balance = web3.eth
          .getBalance(account_one)
          .toNumber();
        account_two_eth_ending_balance = web3.eth
          .getBalance(account_two)
          .toNumber();
        account_three_eth_ending_balance = web3.eth
          .getBalance(account_three)
          .toNumber();

        // check if eth is updated for 2nd and remains the same for first
        assert.equal(
          account_one_eth_ending_balance,
          account_one_eth_starting_balance,
          'Eth balance is not the same'
        );
        assert.equal(
          account_two_eth_ending_balance,
          account_two_eth_starting_balance + eth_amount,
          'Eth balance was not update correctly'
        );
        assert.equal(
          account_three_eth_ending_balance,
          account_three_eth_starting_balance,
          'Eth balance is not the same when eth amount passed was more then balance'
        );
      });
  });
});
