...
class RelayProvider {
  ...
  constructor(origProvider, relayOptions) {
    ...
    if (origProvider.origProvider) {
      ...
      console.log(
        'wrapping RelayProvider with another.. skipping previous one.'
      );
      origProvider = origProvider.origProvider;
    }
    this.origProviderSend = (
      this.origProvider['sendAsync'] || this.origProvider['send']
    ).bind(this.origProvider);
    ...
    if (!this.skipRelay(payload)) {
      if (payload.method == 'eth_sendTransaction') {
        ...
        this.origProviderSend(payload, (e, r) => {
          if (e) callback(e);
          else callback(null, this.relayClient.fixTransactionReceiptResp(r));
        });
        return;
      }
    }
  ...
  skipRelay(payload) {
    return (
      !this.relayOptions.isRelayEnabled ||
      (this.relayOptions.skipSenders &&
        this.relayOptions.skipSenders[payload.params.from])
    );
  }
}
